...
def get_latest_changelog():
    ...
    # Split the document precisely when the first main Engineering update header format is encountered
    sections = re.split(r'^(?=##\s+🛠️\s+Engineering Update)', content, flags=re.MULTILINE)
    
    if len(sections) < 2:
        if not sections:
            return None
        latest_block = sections[0]
    else:
        # If text exists before the first header, skip it and pick the first true update section
        latest_block = sections[1] if sections[0].strip() == "" else sections[0]
